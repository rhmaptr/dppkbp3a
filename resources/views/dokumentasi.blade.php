        ...
        <div class="bg-transparent w-full h-screen ">
            ...
            <div class="bg-transparent w-[1100px] h-[550px] mt-10 ml-8 rounded-lg shadow-lg flex-col">
                ...
                <div class="bg-transparent w-full h-[35px] font-semibold text-[20px] pl-3 flex">
                    ...
                    <script>
                    document.getElementById("tambah_dokumentasi").addEventListener("click", function() {
                        window.location.href = "/tambah_dokumentasi";
                    });
                    </script>
                </div>
                <!-- bagian tabel -->
                <body>
                    <table>
                        ...
                        <tbody>
                            @forelse ($photos as $poto)
                            <tr>
                                <td>{{ $poto->id }}</td>
                                <td>{{ $poto->image }}</td>
                                <td>{{ $poto->title }}</td>
                                <td>
                                    <a href="{{ route('dokumentasi.edit', $poto->id) }}"
                                        class="btn btn-warning">Edit</a>
                                    <form action="{{ route('dokumentasi.destroy', $poto->id) }}" method="POST"
